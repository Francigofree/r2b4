...
def test_prompt_v4_explains_stopped_and_unavailable_semantics(tmp_path: Path):
    path = tmp_path / "system.md"
    path.write_text("SYSTEM V4", encoding="utf-8")
    ...
    messages = assembler.build_messages(
        UserTextTurn("t", "Mi az állapotod?", "cli", 1),
        context,
        (),
    )
    assert PROMPT_VERSION == "R2B4_VOICE_LLM_SYSTEM_V4"
