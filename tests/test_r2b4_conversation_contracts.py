...
from r2b4_voice.conversation_contracts import LLMDecision, RobotAction, RobotContextSnapshot, UserTextTurn
...
def _context(available=True):
    return RobotContextSnapshot(
        ...
        safety=None,
        health=(),
        available_actions=({"name": "v3.command.face_person", "available": available, "ready": True},),
    )
...
def test_action_validator_rejects_unavailable_action():
    result = RobotActionValidator().validate(RobotAction("v3.command.face_person"), _context(False))
...
def test_action_validator_rejects_out_of_range_parameter():
    result = RobotActionValidator().validate(
        RobotAction("v3.command.face_person", (("max_omega_rad_s", 1.0),)),
        _context(),
    )
    assert result.accepted is False
    assert "OUT_OF_RANGE" in result.reason


def test_journal_is_append_only_ndjson_and_0600(tmp_path: Path):
    journal = ConversationJournal(tmp_path / "conversations", session_id="test_session")
