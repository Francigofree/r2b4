...
from v3.adapters.live_lidar import LidarHealthReading, LidarMatcherDiagnostics, LidarPointReading, LidarPoseReading, LidarScanReading, NativeLidarConfig, NativeLidarSource
...
def test_native_lidar_source_closes_health_and_pose_from_one_matcher_result():
    ...
    assert tuple((sample.kind for sample in snapshot.samples)) == ('lidar_health', 'lidar_safety_clearance', 'lidar_local_points', 'lidar_localization_health', 'lidar_matcher_diagnostics', 'lidar_pose')
    ...
    assert {field.key: field.value for field in local.values} == {'frame_id': 'ROBOT_BASE', 'point_count': 2, 'point_000_x_m': 1.0, 'point_000_y_m': 0.0, 'point_000_quality': 15, 'point_001_x_m': pytest.approx(0.0, abs=1e-12), 'point_001_y_m': -0.5, 'point_001_quality': 10}
...
def test_physical_scan_staleness_degrades_device_but_keeps_localization_separate():
    ...
    snapshot = NativeLidarSource(_Backend(dataclasses.replace(_reading(), scan=stale_scan)), _config()).read(TickContext(7, 1000))
