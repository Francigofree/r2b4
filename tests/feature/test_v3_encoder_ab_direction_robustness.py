...
import pytest
from v3.adapters.gpio_counter import GpioCounterChannelConfig, GpioCounterPairConfig, NativeGpioSignedCounterPair
...
def robust_config(*, guard_us=50, confirm_edges=3):
    return GpioCounterPairConfig(left=GpioCounterChannelConfig(17, 18, forward_b_level=1, a_debounce_micros=150, direction_guard_micros=guard_us, direction_change_confirm_edges=confirm_edges, direction_change_confirm_window_micros=250000), right=GpioCounterChannelConfig(22, 23, forward_b_level=1), gpio_chip=2)

def a(gpio, physical_ns, *, pin=17):
    gpio.emit(pin, 1, physical_ns + 150000)
...
def test_three_consistent_opposite_candidates_confirm_real_reversal_retroactively():
    ...
    assert tuple((edge.timestamp_ns for edge in snap.edge_history[-3:])) == (3000000, 4000000, 5000000)
...
def test_raw_ab_diagnostic_trace_preserves_callback_and_physical_a_time():
    gpio = FakeGpio({18: 1, 23: 1})
    owner = NativeGpioSignedCounterPair(gpio, robust_config())
    gpio.emit(18, 0, 900000)
    a(gpio, 1000000)
    events = owner.diagnostic_events('left')
    assert events[0].channel == 'B'
    assert events[-1].channel == 'A'
    assert events[-1].callback_timestamp_ns == 1150000
    assert events[-1].physical_timestamp_ns == 1000000
