...
def _raw_devices_with_local_perception(context: TickContext) -> RawDeviceBatch:
    raw = _raw_devices(context)
    local = DeviceSample(
        device_id="native-lidar",
    ...
    return RawDeviceBatch(context, raw.samples + (local,), raw.device_health)
...
def test_explore_command_uses_the_single_native_l1_l12_trajectory_path():
    writer = RecordingMotorWriter()
    composition = NativeControlComposition(writer, _config())
    results = []
    for tick_id in range(2):
        context = _context(tick_id)
        command = CommandRequest(
            context,
            "room-cruise-upper-consumer",
            CommandMode.EXPLORE,
            (DataField("max_v_mps", 0.10), DataField("max_omega_rad_s", 0.20)),
            expiry_tick=2,
        )
        results.append(
            composition.run_tick(
                TickInputs(
                    context,
                    _raw_devices_with_local_perception(context),
                    command,
                    LifecycleState.ACTIVE,
                )
            )
    ...
    layers = {record.layer: record.output for record in results[-1].trace.layers}
    assert tuple(layers) == tuple(f"L{index}" for index in range(1, 13))
    assert isinstance(layers["L6"], NavigationPlan)
    assert len(layers["L6"].trajectory_candidates) == 54
    assert isinstance(layers["L7"], MotionObjective)
    ...
    assert layers["L6"].local_goal.y_m == 0.0
