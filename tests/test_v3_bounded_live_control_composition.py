...
def test_bounded_explore_profile_uses_the_same_preflight_and_l1_l12_runtime():
    config = replace(
        _config(),
        command_profile=BoundedExploreProfile(
            command_id="room-cruise-consumer",
            start_tick_id=1,
            active_tick_count=2,
            max_v_mps=0.10,
            max_omega_rad_s=0.20,
    ...
    writer = RecordingMotorWriter()
    sources = _sources(EncoderBackend(), ImuBackend(), LidarBackend())
    composition = BoundedLiveControlComposition(*sources, writer, config)

    preflight = composition.tick(_context(0))
    active = composition.tick(_context(1))
    layers = {record.layer: record.output for record in active.trace.layers}

    assert preflight.final_actuation.safety_decision is SafetyDecision.STOP
    assert active.final_actuation.safety_decision is SafetyDecision.ALLOW
    assert len(layers["L6"].trajectory_candidates) == 54
    assert layers["L7"].selected_source == "navigation.trajectory"
