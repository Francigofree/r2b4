...
def test_owned_source_closes_gpio_counts_and_emits_one_typed_snapshot_per_tick():
    ...
    assert (values["left_mps"], values["right_mps"], values["trust"]) == (
        0.0,
        0.0,
        0.0,
    )
...
def test_edge_history_capacity_must_cover_velocity_window_before_gpio_open():
    gpio = GpioBackend()

    with pytest.raises(ValueError, match="cannot cover"):
        NativeGpioEncoderSource(
            gpio,
            replace(_counter_config(), edge_history_capacity=2),
            _backend_config(),
            NativeEncoderConfig("encoder", 0.5),
        )
