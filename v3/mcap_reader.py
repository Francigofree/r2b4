    ...
    def inspect(self, *, verify_chunks: bool = True) -> McapStructureReport:
        ...
        if verify_chunks and chunk_crc_ok:
            try:
                ...
                if counts != self.statistics.get("channel_message_counts"):
                    errors.append("indexed message counts disagree with statistics")
    ...
    def capture_integrity(self) -> dict[str, object]:
        ...
        for message in self.iter_messages():
            if final is not None:
                raise McapReadError("messages follow capture_finalized")
            if message.topic == EVENT_TOPIC:
                value = message.json()
                if isinstance(value, dict) and value.get("event_type") == "capture_finalized":
                    final = value
                    continue
            ...
            digest.update(len(topic).to_bytes(4, "little"))
            digest.update(topic)
            digest.update(message.sequence.to_bytes(4, "little"))
            digest.update(message.log_time_ns.to_bytes(8, "little"))
            digest.update(len(message.data).to_bytes(8, "little"))
            digest.update(message.data)
        if final is None:
            raise McapReadError("CAPTURE_INCOMPLETE: no finalization event")
        integrity = final.get("integrity")
        metadata = self.latest_metadata("r2b4.capture.final")
        if not isinstance(integrity, dict) or metadata is None:
            raise McapReadError("CAPTURE_INCOMPLETE: no final integrity")
        actual = digest.hexdigest()
        if actual != integrity.get("message_stream_sha256") or actual != metadata.get("message_stream_sha256"):
            raise McapReadError("capture message digest mismatch")
        for topic, field in ((TICK_TOPIC, "captured_tick_count"),
                             (RAW_LIDAR_TOPIC, "captured_raw_lidar_count"),
                             (CHECKPOINT_TOPIC, "captured_checkpoint_count")):
            if final.get(field) != counts.get(topic, 0):
                raise McapReadError("capture message count mismatch: " + topic)
        subscription = integrity.get("observation_subscription")
        subscription_ok = isinstance(subscription, dict) and (
            subscription.get("mode") == "RELIABLE" and subscription.get("required") is True
            and subscription.get("closed") is True and subscription.get("queued") == 0
...
def _read_record(handle: BinaryIO) -> tuple[int, bytes, int]:
    ...
    if length > 64 * 1024 * 1024:
        raise McapReadError("MCAP record exceeds 64 MiB read bound")
