...
def validate_run(
    ...
    *,
    ...
    index: dict[str, object] = {
        ...
        "source_first": {
            "all_capture_baseline_hashes_match": (
                source_first.get("all_capture_baseline_hashes_match")
                if isinstance(source_first, Mapping)
                else None
            ),
            "covered_file_count": len(source_files) if isinstance(source_files, Mapping) else 0,
        },
        "artifacts": artifacts,
    }
...
def _diagnosis(
    ...
    source_manifest_present = bool(
        isinstance(source_first, Mapping)
        and source_first.get("capture_source_manifest_path") is not None
    )
    source_match = bool(
        isinstance(source_first, Mapping)
        and source_first.get("all_capture_baseline_hashes_match") is True
    )
    status = (
        "PASS"
        if replay.get("status") == V3_REPLAY_STATUS_MATCH
        and (not source_manifest_present or source_match)
        else "FAIL"
    )
    return {
