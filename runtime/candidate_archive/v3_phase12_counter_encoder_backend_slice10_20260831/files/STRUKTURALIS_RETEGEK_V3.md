...
12. **Első kontrollált V3 fizikai motion és navigation — folyamatban:** az első,
    ...
    injektált, már futó signed pulse-counterből constructionkor egy baseline-t,
    majd minden `TickContext` értéknél counterenként pontosan egy immutable
    snapshotot olvas. A bal/jobb sebességet kizárólag a signed count-delta, az
    immutable oldalankénti step distance és a tick monotonic ideje adja; nincs
    PWM- vagy command-context. A counter running/error diagnosztika, a maximum
    mintaintervallum és a maximum fizikai sebesség determinisztikus trust/stale/
    timing gate-re zár; stale vagy fizikailag lehetetlen minta nullázott, és
    minden növekvő tick újrahorgonyozza a következő deltát. A native snapshot-
    contractot direction, reanchor, stale recovery, diagnosztikai degradáció,
    timing, velocity gate és `NativeEncoderSource` integráció fedi. A backend
    nem importál legacy drivert/service-t vagy global configot, nem nyit GPIO-t,
    nem birtokol callbacket, threadet, órát, runtime-ot vagy writert. A valódi
    GPIO counter-owner, native IMU/LiDAR backend és ezek friss preflight utáni
    élő tesztje továbbra is külön későbbi kapu.
