...
def test_navigation_scenario_replay_is_deterministic_and_progresses_l5_to_l9():
    ...
    assert all(
        trace.objective.kind is MotionObjectiveKind.TRACK_TRAJECTORY
        for trace in first
    )
...
def test_explore_state_is_continuous_while_trajectory_replans_at_10_hz():
    goal = (
        DataField("max_v_mps", 0.3),
        DataField("max_omega_rad_s", 0.6),
    )
    positions = (0.0, 0.1, 0.2, 0.3, 0.4, 0.9)
    frames = tuple(
        _frame(
            tick_id,
            ...
            goal=goal,
            command_id="resident-room-cruise",
            x_m=x_m,
        )
    ...
    assert {trace.mission.mission_id for trace in traces} == {
        "mission-resident-room-cruise"
    }
    assert tuple(plan.context for plan in plans) == tuple(frame.context for frame in frames)
    assert all(plan.trajectory_candidates == plans[0].trajectory_candidates for plan in plans[1:5])
    assert plans[5].trajectory_candidates != plans[4].trajectory_candidates
    assert all(plan.local_goal == plans[0].local_goal for plan in plans[1:5])
    assert plans[5].local_goal != plans[4].local_goal
    assert tuple(plan.progress for plan in plans) == tuple(
        sorted(plan.progress for plan in plans)
    )
    assert plans[-1].progress > plans[0].progress
    assert all(trace.objective.kind is MotionObjectiveKind.TRACK_TRAJECTORY for trace in traces)
