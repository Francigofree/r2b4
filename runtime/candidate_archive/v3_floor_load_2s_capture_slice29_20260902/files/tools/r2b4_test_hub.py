...
V3_NATIVE_FLOOR_LOAD_2S_PROFILE = "v3_native_floor_load_2s_capture"
...
V3_NATIVE_FLOOR_LOAD_2S_COMMAND = "v3-native-floor-load-2s-capture"
...
V3_NATIVE_FLOOR_LOAD_2S_APPROVAL = (
    "floor-clear-0p70m-speed-0p15-duration-2p0-v3"
)
...
V3_NATIVE_FLOOR_LOAD_2S_SCHEMA = "R2B4_V3_NATIVE_FLOOR_LOAD_2S_CAPTURE_V1"
...
V3_NATIVE_FLOOR_LOAD_2S_ACTIVE_TICK_COUNT = 100
V3_NATIVE_FLOOR_LOAD_2S_V_MPS = 0.15
V3_NATIVE_FLOOR_LOAD_2S_PREFLIGHT_CLEARANCE_M = 0.70
V3_NATIVE_FLOOR_LOAD_2S_ACTIVE_CLEARANCE_M = 0.30
V3_NATIVE_FLOOR_LOAD_2S_PREFLIGHT_CLEAR_SCAN_COUNT = 2
V3_NATIVE_FLOOR_LOAD_2S_MAX_DISPLACEMENT_M = 0.45
V3_NATIVE_FLOOR_LOAD_2S_MAX_YAW_DELTA_RAD = 0.35
V3_NATIVE_FLOOR_LOAD_2S_MAX_ACTIVE_DURATION_S = 2.50
V3_NATIVE_FLOOR_LOAD_2S_MAX_ENCODER_ABS_MPS = 0.45
...
@dataclass(frozen=True)
class _V3FloorMotionSpec:
    profile: str
    approval: str
    schema: str
    command_prefix: str
    artifact_filename: str
    tick_capture_filename: str
    active_tick_count: int
    v_mps: float
    preflight_clearance_m: float
    active_clearance_m: float
    preflight_clear_scan_count: int
    max_displacement_m: float
    max_yaw_delta_rad: float
    min_active_duration_s: float
    max_active_duration_s: float
    max_encoder_abs_mps: float


V3_NATIVE_FLOOR_MOTION_SPEC = _V3FloorMotionSpec(
    profile=V3_NATIVE_FLOOR_MOTION_PROFILE,
    approval=V3_NATIVE_FLOOR_MOTION_APPROVAL,
    schema=V3_NATIVE_FLOOR_MOTION_SCHEMA,
    command_prefix="v3-native-floor-motion",
    artifact_filename="v3_native_floor_motion_capture.json",
    tick_capture_filename="v3_native_floor_motion_ticks.json",
    ...
    preflight_clear_scan_count=V3_NATIVE_FLOOR_PREFLIGHT_CLEAR_SCAN_COUNT,
    max_displacement_m=V3_NATIVE_FLOOR_MAX_DISPLACEMENT_M,
    max_yaw_delta_rad=V3_NATIVE_FLOOR_MAX_YAW_DELTA_RAD,
    min_active_duration_s=0.90,
    max_active_duration_s=V3_NATIVE_FLOOR_MAX_ACTIVE_DURATION_S,
    max_encoder_abs_mps=V3_NATIVE_FLOOR_MAX_ENCODER_ABS_MPS,
)

V3_NATIVE_FLOOR_LOAD_2S_SPEC = _V3FloorMotionSpec(
    profile=V3_NATIVE_FLOOR_LOAD_2S_PROFILE,
    approval=V3_NATIVE_FLOOR_LOAD_2S_APPROVAL,
    schema=V3_NATIVE_FLOOR_LOAD_2S_SCHEMA,
    command_prefix="v3-native-floor-load-2s",
    artifact_filename="v3_native_floor_load_2s_capture.json",
    tick_capture_filename="v3_native_floor_load_2s_ticks.json",
    active_tick_count=V3_NATIVE_FLOOR_LOAD_2S_ACTIVE_TICK_COUNT,
    v_mps=V3_NATIVE_FLOOR_LOAD_2S_V_MPS,
    preflight_clearance_m=V3_NATIVE_FLOOR_LOAD_2S_PREFLIGHT_CLEARANCE_M,
    active_clearance_m=V3_NATIVE_FLOOR_LOAD_2S_ACTIVE_CLEARANCE_M,
    preflight_clear_scan_count=V3_NATIVE_FLOOR_LOAD_2S_PREFLIGHT_CLEAR_SCAN_COUNT,
    max_displacement_m=V3_NATIVE_FLOOR_LOAD_2S_MAX_DISPLACEMENT_M,
    max_yaw_delta_rad=V3_NATIVE_FLOOR_LOAD_2S_MAX_YAW_DELTA_RAD,
    min_active_duration_s=1.90,
    max_active_duration_s=V3_NATIVE_FLOOR_LOAD_2S_MAX_ACTIVE_DURATION_S,
    max_encoder_abs_mps=V3_NATIVE_FLOOR_LOAD_2S_MAX_ENCODER_ABS_MPS,
)
...
def _scenario_registry() -> Dict[str, ScenarioProfile]:
    py = sys.executable
    return {
        V3_NATIVE_FLOOR_LOAD_2S_PROFILE: ScenarioProfile(
            name=V3_NATIVE_FLOOR_LOAD_2S_PROFILE,
            family="v3_native_hardware",
            description=(
                "Health-armed two-second native V3 floor-load measurement with "
                "current raw LiDAR clearance, full L1-L12 capture and bounded shutdown."
            ),
            live=True,
            timeout_s=75.0,
            command=(
                ...
                V3_NATIVE_FLOOR_LOAD_2S_COMMAND,
            ),
            preflight_clearance_m=V3_NATIVE_FLOOR_LOAD_2S_PREFLIGHT_CLEARANCE_M,
            ...
            goals=(
                ...
                "current raw LiDAR front-sector clearance of at least 0.70 m before health arming",
                "exactly 100 resident ticks at 0.15 m/s nominal straight speed (2.0 s at 20 ms)",
...
def _v3_floor_profile_artifact_paths(
    spec: _V3FloorMotionSpec = V3_NATIVE_FLOOR_MOTION_SPEC,
) -> Tuple[Path, Path]:
    if os.environ.get(V3_NATIVE_PROFILE_ENV_VAR) != spec.profile:
        raise PermissionError(
    ...
    return (
        directory / spec.artifact_filename,
        directory / spec.tick_capture_filename,
    )
...
def _run_v3_native_floor_motion_capture(
    approval: str,
    spec: _V3FloorMotionSpec = V3_NATIVE_FLOOR_MOTION_SPEC,
) -> Dict[str, Any]:
    ...
    artifact_path, tick_capture_path = _v3_floor_profile_artifact_paths(spec)
    lease_gate = _verify_v3_motion_leases()
    recorder: Optional[_V3MotorGpioRecorder] = None
    expected_pins: Tuple[int, ...] = (12, 13, 18, 19)
    stop = _V3SignalStop()
    ...
    try:
        ...
        recorder = _V3MotorGpioRecorder(api["motor_gpio"])
        gateway = _V3ResidentRaisedStandGateway(
            active_tick_count=spec.active_tick_count,
            v_mps=spec.v_mps,
            max_v_mps=spec.v_mps,
        ...
        def observe_tick(result: Any) -> None:
            ...
            clearance, raw_capture = _v3_lidar_tick_evidence(
                ...
                tick_ns,
                clearance_m=(
                    spec.preflight_clearance_m
                    if gateway.active_tick_id is None
                    else spec.active_clearance_m
                ),
            )
            ...
            if gateway.active_tick_id is None:
                ...
                if scan_id > 0 and scan_id != last_clearance_scan_id:
                    ...
                    if clearance.get("ok") is True:
                        ...
                        del clear_scan_ids[:-spec.preflight_clear_scan_count]
                        del clear_scan_evidence[:-spec.preflight_clear_scan_count]
                    else:
            ...
            schedule = gateway.observe(
                summary,
                arm_permitted=(
                    len(clear_scan_ids) >= spec.preflight_clear_scan_count
                ),
            )
            if schedule == "ARMED":
            ...
            if active:
                ...
                if encoder_abs_mps is None:
                    ...
                    if encoder_abs_mps > spec.max_encoder_abs_mps:
                        blockers.append(
                            {
                                "code": "ACTIVE_ENCODER_SPEED_BOUND",
                                "measured_mps": encoder_abs_mps,
                                "limit_mps": spec.max_encoder_abs_mps,
                            }
                        )
                ...
                if not baseline_estimate or not estimate:
                    ...
                    if displacement_m > spec.max_displacement_m:
                        blockers.append(
                            {
                                "code": "ACTIVE_L3_DISPLACEMENT_BOUND",
                                "measured_m": displacement_m,
                                "limit_m": spec.max_displacement_m,
                            }
                        )
                    if yaw_delta_rad > spec.max_yaw_delta_rad:
                        blockers.append(
                            {
                                "code": "ACTIVE_L3_YAW_BOUND",
                                "measured_rad": yaw_delta_rad,
                                "limit_rad": spec.max_yaw_delta_rad,
                            }
                        )
                active_elapsed_s = (
                    tick_ns - int(active_start_monotonic_ns)
                ) / 1_000_000_000.0
                if active_elapsed_s > spec.max_active_duration_s:
                    blockers.append(
                        {
                            "code": "ACTIVE_ELAPSED_TIME_BOUND",
                            "measured_s": active_elapsed_s,
                            "limit_s": spec.max_active_duration_s,
                        }
                    )
        ...
        success = bool(
            ...
            and tick_evidence.get("resident_preflight") is not None
            and len(clear_scan_ids) >= spec.preflight_clear_scan_count
            and allow_tick_ids == expected_allow_tick_ids
            and len(allow_tick_ids) == spec.active_tick_count
            ...
            and spec.min_active_duration_s <= float(active_duration_s)
            <= spec.max_active_duration_s
        ...
        payload = {
            ...
            "safety_bounds": {
                "preflight_clearance_m": spec.preflight_clearance_m,
                "active_clearance_m": spec.active_clearance_m,
                "maximum_displacement_m": spec.max_displacement_m,
                "maximum_abs_yaw_delta_rad": spec.max_yaw_delta_rad,
                "minimum_active_duration_s": spec.min_active_duration_s,
                "maximum_active_duration_s": spec.max_active_duration_s,
                "maximum_encoder_abs_mps": spec.max_encoder_abs_mps,
            },
...
def _run_v3_native_floor_load_2s_capture(approval: str) -> Dict[str, Any]:
    return _run_v3_native_floor_motion_capture(
        approval,
        V3_NATIVE_FLOOR_LOAD_2S_SPEC,
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_v3_floor_load_2s = sub.add_parser(
        V3_NATIVE_FLOOR_LOAD_2S_COMMAND,
        help=argparse.SUPPRESS,
    )
    p_v3_floor_load_2s.add_argument("--approval", required=True)
...
def main() -> int:
    ...
    if command == V3_NATIVE_FLOOR_LOAD_2S_COMMAND:
        payload = _run_v3_native_floor_load_2s_capture(str(args.approval))
