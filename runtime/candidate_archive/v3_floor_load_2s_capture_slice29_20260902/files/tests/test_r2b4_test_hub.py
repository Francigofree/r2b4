...
class TestR2B4TestHub(unittest.TestCase):
    ...
    def test_floor_load_2s_v3_profile_and_approval_gate_are_explicit(self):
        profile = hub.SCENARIOS[hub.V3_NATIVE_FLOOR_LOAD_2S_PROFILE]

        self.assertTrue(profile.live)
        self.assertFalse(profile.requires_managed_runtime)
        self.assertEqual(profile.preflight_kind, hub.V3_NATIVE_PREFLIGHT_KIND)
        self.assertEqual(profile.preflight_clearance_m, 0.70)
        self.assertEqual(profile.command[-1], hub.V3_NATIVE_FLOOR_LOAD_2S_COMMAND)
        self.assertNotIn(hub.V3_NATIVE_FLOOR_LOAD_2S_APPROVAL, profile.command)
        with mock.patch.object(
            hub,
            "_v3_floor_profile_artifact_paths",
            side_effect=AssertionError("artifact paths must not be evaluated"),
            ...
            result = hub._run_v3_native_floor_load_2s_capture("not-approved")

        self.assertEqual(result["status"], "FAIL")
        self.assertFalse(result["success"])
        self.assertEqual(result["schema"], hub.V3_NATIVE_FLOOR_LOAD_2S_SCHEMA)
        self.assertEqual(
            result["error"],
            "explicit_floor_clearance_and_speed_approval_required",
        )

    def test_floor_load_2s_v3_gateway_schedules_exact_100_ticks_at_0p15_mps(self):
        gateway = hub._V3ResidentRaisedStandGateway(
            active_tick_count=hub.V3_NATIVE_FLOOR_LOAD_2S_ACTIVE_TICK_COUNT,
            v_mps=hub.V3_NATIVE_FLOOR_LOAD_2S_V_MPS,
            max_v_mps=hub.V3_NATIVE_FLOOR_LOAD_2S_V_MPS,
            command_prefix="floor-load-2s-unit",
        )
        healthy = {
            "tick_id": 7,
            "fault_layer": None,
        ...
        self.assertEqual(gateway.post_active_idle_tick_id, 108)
        self.assertEqual(gateway.shutdown_tick_id, 109)
        self.assertEqual(
            hub.V3_NATIVE_FLOOR_LOAD_2S_SPEC.active_tick_count * 0.02,
            2.0,
        )
