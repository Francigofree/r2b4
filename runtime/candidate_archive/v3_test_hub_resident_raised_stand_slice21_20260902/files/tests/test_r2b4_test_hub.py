...
class TestR2B4TestHub(unittest.TestCase):
    ...
    def test_resident_v3_handler_signals_shutdown_and_records_hard_low_pass(self):
        ...
        def result_for(tick_id, *, active=False):
            health = tuple(
                SimpleNamespace(
                    device_id=device,
        ...
        def fake_resident_run(
            ...
            *,
            ...
            for tick_id in range(4):
                ...
                active = tick_id == hub.V3_NATIVE_RESIDENT_ACTIVE_TICK_ID
                self.assertEqual(
                    command.mode,
                ...
                if active:
                    motor_gpio.tx_pwm(handle, 12, 8_000, 0.2)
                    motor_gpio.tx_pwm(handle, 18, 8_000, 0.2)
                if tick_id == hub.V3_NATIVE_RESIDENT_SIGNAL_AFTER_TICK_ID:
                    for pin in pins:
                        if motor_gpio.tx_busy(handle, pin, 0):
                            motor_gpio.tx_pwm(handle, pin, 0, 0.0)
                tick_observer(result_for(tick_id, active=active))
            self.assertTrue(stop_requested())
            tick_observer(
                result_for(hub.V3_NATIVE_RESIDENT_SHUTDOWN_TICK_ID)
            )
            for pin in pins:
                motor_gpio.gpio_write(handle, pin, 0)
            for pin in pins:
            ...
            return SimpleNamespace(
                as_dict=lambda: {
                    ...
                    "tick_count": 5,
                    "normal_tick_count": 4,
                    "last_tick_id": hub.V3_NATIVE_RESIDENT_SHUTDOWN_TICK_ID,
        ...
        self.assertEqual(
            ...
            hub.V3_NATIVE_RESIDENT_SIGNAL_AFTER_TICK_ID,
        )
        self.assertTrue(persisted["motor_gpio"]["all_active_pwm_cancelled"])
        self.assertTrue(persisted["motor_gpio"]["all_final_verified_low"])
        self.assertTrue(persisted["post_close_pins"]["ok"])
