...
V3_NATIVE_RESIDENT_MOTION_SCHEMA = "R2B4_V3_NATIVE_RESIDENT_RAISED_STAND_V1"
V3_NATIVE_START_TICK_ID = 200
V3_NATIVE_ACTIVE_TICK_COUNT = 1
V3_NATIVE_V_MPS = 0.04
V3_NATIVE_RESIDENT_ACTIVE_TICK_ID = 2
V3_NATIVE_RESIDENT_SIGNAL_AFTER_TICK_ID = 3
V3_NATIVE_RESIDENT_SHUTDOWN_TICK_ID = 4
...
class _V3ResidentRaisedStandGateway:
    """Fixed Test Hub command edge; it owns no physical output capability."""

    def snapshot(self, context: Any) -> Any:
        from v3.contracts import CommandMode, CommandRequest, DataField

        active = int(context.tick_id) == V3_NATIVE_RESIDENT_ACTIVE_TICK_ID
        return CommandRequest(
            context=context,
            command_id=f"v3-resident-raised-stand.{int(context.tick_id)}",
            mode=CommandMode.TELEOP if active else CommandMode.STOP,
            goal=(
                (
                    DataField("v_mps", V3_NATIVE_RESIDENT_V_MPS),
                    DataField("omega_rad_s", 0.0),
                    DataField("max_v_mps", 0.05),
...
def _run_v3_native_resident_raised_stand_motion(approval: str) -> Dict[str, Any]:
    ...
    try:
        ...
        def observe_tick(result: Any) -> None:
            ...
            if (
                tick_id == V3_NATIVE_RESIDENT_ACTIVE_TICK_ID - 1
                and summary["fault_layer"] is None
                and summary["safety_decision"] == "STOP"
                and summary["safety_reason"] == "NOT_ACTIVE"
                and summary["source_health"]
                and all(
                    row.get("state") == "OK"
            ...
            if (
                ...
                tick_evidence["first_fault"] = summary
            if tick_id == V3_NATIVE_RESIDENT_SIGNAL_AFTER_TICK_ID:
                tick_evidence["post_active_idle"] = summary
                if (
                    summary["fault_layer"] is None
                    and summary["safety_decision"] == "STOP"
                    and summary["safety_reason"] == "NOT_ACTIVE"
                    and summary["enabled"] is False
                    and summary["left_output"] == 0.0
                    and summary["right_output"] == 0.0
                    and tick_evidence["signal_raised_after_tick"] is None
                ):
                    tick_evidence["signal_raised_after_tick"] = tick_id
                    signal.raise_signal(signal.SIGTERM)
            if tick_id == V3_NATIVE_RESIDENT_SHUTDOWN_TICK_ID:
        ...
        success = bool(
            ...
            and report_payload.get("normal_tick_count") == 4
            and report_payload.get("tick_count") == 5
            and report_payload.get("last_tick_id")
            == V3_NATIVE_RESIDENT_SHUTDOWN_TICK_ID
        ...
        payload = {
            ...
            "operator_stopped": stop.requested,
            "command_window": {
                "active_tick_id": V3_NATIVE_RESIDENT_ACTIVE_TICK_ID,
                "active_tick_count": 1,
                "signal_after_tick_id": V3_NATIVE_RESIDENT_SIGNAL_AFTER_TICK_ID,
                "shutdown_tick_id": V3_NATIVE_RESIDENT_SHUTDOWN_TICK_ID,
