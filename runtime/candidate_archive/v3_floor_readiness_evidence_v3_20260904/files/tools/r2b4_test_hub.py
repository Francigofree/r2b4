...
V3_NATIVE_FLOOR_MOTION_SCHEMA = "R2B4_V3_NATIVE_FLOOR_MOTION_CAPTURE_V3"
...
V3_NATIVE_FLOOR_REQUIRED_LIDAR_REVISIONS = 3
V3_NATIVE_FLOOR_ACTIVE_SAFETY_THRESHOLD_M = 0.20
...
def _v3_terminal_safe_low_evidence(
    report: Mapping[str, Any],
    motor: Mapping[str, Any],
    post_close_pins: Mapping[str, Any],
) -> Dict[str, Any]:
    """Classify a resident termination without hiding a faulted run."""

    classification = str(report.get("termination_class", "") or "UNVERIFIED")
    runtime_safe_low = classification in (
        "SHUTDOWN_SAFE_LOW",
        "FAULT_SAFE_LOW",
    )
    motor_safe_low = bool(
        motor.get("all_expected_pins_claimed_low") is True
    ...
    post_close_safe_low = post_close_pins.get("ok") is True
    return {
        "classification": classification,
        "verified": bool(
            runtime_safe_low and motor_safe_low and post_close_safe_low
        ),
        "runtime_reported_safe_low": runtime_safe_low,
        "motor_gpio_verified_low": motor_safe_low,
        "post_close_pins_verified_low": post_close_safe_low,
    }
...
def _v3_tick_result_summary(result: Any) -> Dict[str, Any]:
    ...
    lidar_health: Dict[str, Any] = {}
    lidar_matcher_diagnostics: Dict[str, Any] = {}
    for record in trace.layers:
        if record.layer == "L1":
            ...
            for sample in tuple(getattr(record.output, "samples", ()) or ()):
                ...
                if kind not in ("lidar_health", "lidar_matcher_diagnostics"):
                    continue
                values = {
                    str(getattr(item, "key", "")): _v3_capture_value(
                ...
                evidence = {
                    "device_id": str(getattr(sample, "device_id", "")),
                    "revision": int(getattr(sample, "sequence", 0)),
                    "captured_monotonic_ns": int(
                        getattr(sample, "captured_monotonic_ns", 0)
                    ),
                    **values,
                }
                if kind == "lidar_health":
                    lidar_health = evidence
                else:
                    lidar_matcher_diagnostics = evidence
    ...
    return {
        ...
        "lidar_matcher_diagnostics": lidar_matcher_diagnostics,
        "estimate": estimate,
    }
...
class _V3ResidentRaisedStandGateway:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            not isinstance(required_lidar_preflight_revisions, int)
            or isinstance(required_lidar_preflight_revisions, bool)
            or not 1 <= required_lidar_preflight_revisions <= 32
        ):
            raise ValueError(
        ...
        self.required_lidar_preflight_revisions = (
            required_lidar_preflight_revisions
        )
        self.last_lidar_preflight_revision: Optional[int] = None
        self.lidar_preflight_revision_count = 0
    ...
    def observe(
        ...
        *,
        ...
        lidar_health = dict(summary.get("lidar_health") or {})
        lidar_revision = int(lidar_health.get("revision", 0) or 0)
        healthy_idle = bool(
            tick_id >= 0
            ...
            and lidar_revision > 0
        )
        if healthy_idle:
            previous = self.last_lidar_preflight_revision
            if previous is None:
                self.lidar_preflight_revision_count = 1
            elif lidar_revision > previous:
                self.lidar_preflight_revision_count = min(
                    self.required_lidar_preflight_revisions,
                    self.lidar_preflight_revision_count + 1,
                )
            elif lidar_revision < previous:
                self.lidar_preflight_revision_count = 1
            self.last_lidar_preflight_revision = lidar_revision
        else:
            self.last_lidar_preflight_revision = None
            self.lidar_preflight_revision_count = 0
        if (
            healthy_idle
            and self.lidar_preflight_revision_count
            >= self.required_lidar_preflight_revisions
            and arm_permitted
        ):
...
def _v3_native_resident_runtime_config(
    *,
    required_lidar_preflight_revisions: int = 3,
) -> Any:
...
def _run_v3_native_floor_motion_capture(approval: str) -> Dict[str, Any]:
    ...
    try:
        ...
        config = _v3_native_resident_runtime_config(
            required_lidar_preflight_revisions=(
                V3_NATIVE_FLOOR_REQUIRED_LIDAR_REVISIONS
            ),
        )
        active_safety_threshold_m = float(
            config.composition.live_control.control.motion_realization.local_clearance_m
        )
        if not math.isclose(
            active_safety_threshold_m,
            V3_NATIVE_FLOOR_ACTIVE_SAFETY_THRESHOLD_M,
            rel_tol=0.0,
            abs_tol=1e-12,
        ):
            raise RuntimeError(
                "native V3 active safety threshold changed: "
                f"{active_safety_threshold_m!r}"
            )
        expected_pins = tuple(config.composition.motor_output.pins)
        recorder = _V3MotorGpioRecorder(api["motor_gpio"])
        gateway = _V3ResidentRaisedStandGateway(
        ...
        lidar_matcher_preflight_revisions: List[Dict[str, Any]] = []
        ...
        def observe_tick(result: Any) -> None:
            ...
            previous_lidar_revision = gateway.last_lidar_preflight_revision
            ...
            current_lidar_revision = gateway.last_lidar_preflight_revision
            if current_lidar_revision is None:
                lidar_matcher_preflight_revisions.clear()
            elif current_lidar_revision != previous_lidar_revision:
                if (
                    previous_lidar_revision is not None
                    and current_lidar_revision < previous_lidar_revision
                ):
                    lidar_matcher_preflight_revisions.clear()
                lidar_matcher_preflight_revisions.append(
                    {
                        "tick_id": tick_id,
                        "revision": current_lidar_revision,
                        "lidar_health": dict(summary.get("lidar_health") or {}),
                        "lidar_matcher_diagnostics": dict(
                            summary.get("lidar_matcher_diagnostics") or {}
                        ),
                    }
                )
            if schedule == "ARMED":
        ...
        motor = _v3_motor_gpio_evidence(recorder, expected_pins)
        post_close_pins = _v3_post_close_pin_state(expected_pins)
        terminal_safe_low = _v3_terminal_safe_low_evidence(
            report_payload,
            motor,
            post_close_pins,
        )
        ...
        preflight_revisions = [
            ...
            for item in lidar_matcher_preflight_revisions
        ]
        preflight_revisions_strictly_increasing = bool(
            len(preflight_revisions)
            >= V3_NATIVE_FLOOR_REQUIRED_LIDAR_REVISIONS
            and all(
                current > previous
                for previous, current in zip(
                    preflight_revisions,
                    preflight_revisions[1:],
                )
            )
        )
        success = bool(
            ...
            and gateway.lidar_preflight_revision_count
            >= V3_NATIVE_FLOOR_REQUIRED_LIDAR_REVISIONS
            and preflight_revisions_strictly_increasing
            ...
            and int(_safe_int(motor.get("failed_event_count"), -1)) == 0
            and post_close_pins.get("ok") is True
            and terminal_safe_low.get("classification")
            == "SHUTDOWN_SAFE_LOW"
            and terminal_safe_low.get("verified") is True
        )
        payload = {
            "schema": V3_NATIVE_FLOOR_MOTION_SCHEMA,
            "status": "PASS" if success else "FAIL",
            ...
            "resident_report": report_payload,
            "operator_stopped": stop.requested,
            "termination_class": terminal_safe_low["classification"],
            "terminal_safe_low": terminal_safe_low,
            "command_window": {
                "maximum_warmup_tick_id": gateway.max_warmup_tick_id,
                "required_lidar_preflight_revisions": (
                    V3_NATIVE_FLOOR_REQUIRED_LIDAR_REVISIONS
                ),
                "observed_lidar_preflight_revision_count": (
                    gateway.lidar_preflight_revision_count
                ),
                "active_tick_id": active_tick_id,
            ...
            "safety_bounds": {
                ...
                "active_safety_threshold_m": active_safety_threshold_m,
                "target_distance_m": V3_NATIVE_FLOOR_TARGET_DISTANCE_M,
