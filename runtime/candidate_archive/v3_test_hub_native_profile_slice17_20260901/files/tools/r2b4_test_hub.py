...
V3_NATIVE_MOTION_APPROVAL = "raised-stand-bounded-v3"
V3_NATIVE_PROFILE_ENV_VAR = "R2B4_TEST_HUB_PROFILE"
V3_AGENT_LEASE_ROOT_ENV_VAR = "R2B4_AGENT_LEASE_ROOT"
V3_NATIVE_MOTION_SCHEMA = "R2B4_V3_NATIVE_RAISED_STAND_MOTION_V1"
...
def _scenario_registry() -> Dict[str, ScenarioProfile]:
    ...
    return {
        V3_NATIVE_RAISED_STAND_PROFILE: ScenarioProfile(
            ...
            description=(
                "Finite native V3 raised-stand validation through the canonical "
                "bounded L12 motor writer."
            ),
            live=True,
            timeout_s=45.0,
            command=(
            ...
            goals=(
                ...
                "explicit per-run raised-stand approval with no approval embedded in the profile",
                "one 20 ms 0.04 m/s straight pulse through the canonical V3 bounded runtime",
                "non-zero wheel PWM proof followed by final IDLE and four-pin PWM null",
...
class _V3MotorGpioRecorder:
    ...
    def gpio_claim_output(self, handle: int, pin: int) -> Any:
        try:
            result = self._backend.gpio_claim_output(handle, pin)
        except Exception as exc:
            self._record(
                "gpio_claim_output",
                handle=int(handle),
...
def _v3_motor_gpio_evidence(
    ...
    successful_pwm = [
        ...
        if event.get("operation") == "tx_pwm" and event.get("ok") is True
    ]
    final_by_pin: Dict[int, float] = {}
    max_abs_by_pin: Dict[int, float] = {pin: 0.0 for pin in pins}
    nonzero_count = 0
    for event in successful_pwm:
        pin = int(event["pin"])
        duty = float(event["duty_cycle"])
        final_by_pin[pin] = duty
        if pin in max_abs_by_pin:
            max_abs_by_pin[pin] = max(max_abs_by_pin[pin], abs(duty))
        if duty != 0.0:
            nonzero_count += 1
    claimed = {
        int(event["pin"])
        for event in recorder.events
        if event.get("operation") == "gpio_claim_output" and event.get("ok") is True
    }
    opens = [
        event
        for event in recorder.events
        if event.get("operation") == "gpiochip_open" and event.get("ok") is True
    ]
    closes = [
        event
        for event in recorder.events
        if event.get("operation") == "gpiochip_close" and event.get("ok") is True
    ]
    last_pwm_sequence = max(
        (int(event["sequence"]) for event in successful_pwm),
        default=-1,
    )
    ...
    final_zero = set(final_by_pin) == set(pins) and all(
        final_by_pin[pin] == 0.0 for pin in pins
    )
    close_after_pwm_null = bool(closes) and last_close_sequence > last_pwm_sequence
    return {
        "expected_pins": list(pins),
        "opened_handle_count": len(opens),
        "claimed_pins": sorted(claimed),
        "pwm_write_count": len(successful_pwm),
        "nonzero_pwm_write_count": nonzero_count,
        "final_pwm_by_pin": {str(pin): final_by_pin.get(pin) for pin in pins},
        "maximum_abs_pwm_by_pin": {str(pin): max_abs_by_pin[pin] for pin in pins},
        "all_expected_pins_claimed": claimed == set(pins),
        "all_final_pwm_zero": final_zero,
        "gpio_closed_after_final_pwm_null": close_after_pwm_null,
...
def _v3_agent_lease_root() -> Path:
    ...
    root = Path(configured).resolve() if configured else PROJECT_ROOT.resolve()
    if not (root / "tools" / "agentctl.py").is_file():
...
def _run_v3_native_raised_stand_motion(approval: str) -> Dict[str, Any]:
    ...
    if approval != V3_NATIVE_MOTION_APPROVAL:
        return {
            ...
            "status": "FAIL",
            "success": False,
            "error": "explicit_raised_stand_approval_required",
        }
    ...
    try:
        ...
        success = bool(
            ...
            and motor.get("all_expected_pins_claimed") is True
            and int(_safe_int(motor.get("nonzero_pwm_write_count"), 0)) > 0
            and motor.get("all_final_pwm_zero") is True
            and motor.get("gpio_closed_after_final_pwm_null") is True
        )
        payload = {
            "schema": V3_NATIVE_MOTION_SCHEMA,
            ...
            "final_lifecycle_basis": (
                ...
                "L12 then wrote zero to all four pins and closed the GPIO handle"
            ),
