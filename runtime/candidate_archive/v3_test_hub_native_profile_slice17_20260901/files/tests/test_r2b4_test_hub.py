...
class TestR2B4TestHub(unittest.TestCase):
    ...
    def test_native_v3_motion_rejects_missing_approval_before_any_hardware_gate(self):
        ...
        self.assertEqual(result["error"], "explicit_raised_stand_approval_required")

    def test_native_v3_motor_evidence_requires_nonzero_then_four_pin_null_and_close(self):
        class Backend:
            def gpiochip_open(self, _chip):
                return 7

            def gpio_claim_output(self, _handle, _pin):
                return 0

            def tx_pwm(self, _handle, _pin, _frequency_hz, _duty_cycle):
                return 0
        ...
        pins = (12, 13, 18, 19)
        for pin in pins:
            recorder.gpio_claim_output(handle, pin)
            recorder.tx_pwm(handle, pin, 8_000, 0.0)
        ...
        self.assertTrue(evidence["all_expected_pins_claimed"])
        self.assertEqual(evidence["nonzero_pwm_write_count"], 2)
        self.assertTrue(evidence["all_final_pwm_zero"])
        self.assertTrue(evidence["gpio_closed_after_final_pwm_null"])
    ...
    def test_native_v3_motion_handler_records_pass_without_bypassing_canonical_runner(self):
        ...
        def fake_canonical_run_owned(
            ...
            *,
            ...
            for pin in pins:
                motor_gpio.gpio_claim_output(handle, pin)
                motor_gpio.tx_pwm(handle, pin, 8_000, 0.0)
        ...
        self.assertTrue(persisted["motor_gpio"]["all_final_pwm_zero"])
