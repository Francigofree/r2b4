...
def create_explore_capture(tmp_path: Path, *, capture_id: str = "explore-v3") -> Path:
    inputs = []
    for base in tick_inputs(4):
        ...
        command = CommandRequest(
            context,
            "room-cruise-replay" if active else f"room-cruise-stop-{tick_id}",
        ...
        inputs.append(
            TickInputs(
                ...
                RawDeviceBatch(
                    context,
                    base.raw_devices.samples + (local,),
                    base.raw_devices.device_health,
                ),
                command,
                LifecycleState.ACTIVE if active else LifecycleState.IDLE,
            )
