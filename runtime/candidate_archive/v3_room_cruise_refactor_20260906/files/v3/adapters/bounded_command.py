...
@dataclass(frozen=True, slots=True)
class BoundedExploreProfile:
    """One immutable EXPLORE request inside one absolute, finite tick window."""
    ...
    def __post_init__(self) -> None:
        ...
        try:
            ...
            raise ValueError("command_id must be a non-empty token") from exc
        _nonnegative_integer(self.start_tick_id, "start_tick_id")
        if _nonnegative_integer(self.active_tick_count, "active_tick_count") == 0:
            raise ValueError("active_tick_count must be positive")
        if _finite(self.max_v_mps, "max_v_mps") <= 0.0:
            raise ValueError("max_v_mps must be positive")
        if _finite(self.max_omega_rad_s, "max_omega_rad_s") <= 0.0:
...
class BoundedExploreCommandGateway:
    """Emit one bounded generic exploration mission and STOP outside it."""

    __slots__ = ("_profile",)

    def __init__(self, profile: BoundedExploreProfile) -> None:
        if not isinstance(profile, BoundedExploreProfile):
            raise TypeError("profile must be BoundedExploreProfile")
        self._profile = profile
