...
def load_v3_navigation_config(path_value: str | Path) -> V3NavigationConfig:
    return v3_navigation_config_from_mapping(
        _load_json_object(path_value, "control config")
    )
...
def _sensor_hardware_config(
    ...
    navigation: V3NavigationConfig | None = None,
) -> NativeSensorHardwareConfig:
    ...
    inputs = NativeSensorInputConfig(
        ...
        lidar_source=NativeLidarConfig(
            ...
            local_perception_min_range_m=(
                navigation.local_perception_min_range_m
                if navigation is not None
                else 0.08
            ),
            local_perception_max_range_m=(
                navigation.local_perception_max_range_m
                if navigation is not None
                else 2.50
            ),
            local_perception_max_points=(
                navigation.local_perception_max_points
                if navigation is not None
                else 96
            ),
        ),
...
def load_bounded_physical_runtime_config(
    ...
    *,
    ...
    if not isinstance(command_profile, (BoundedTeleopProfile, BoundedExploreProfile)):
        raise TypeError("command_profile must be a bounded command profile")
    ...
    navigation_config = (
        load_v3_navigation_config(control_path)
    ...
    sensor_inputs = (
        _sensor_hardware_config(hardware, encoder, sensor_policy, navigation_config)
