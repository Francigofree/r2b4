...
**Runtime-aktiválás:** `NOT_AUTHORIZED`. A dokumentum és a jelenlegi V3 source
nem engedélyez live motiont, legacy runtime-kiváltást vagy fizikai cutovert.
...
3. **L10–L11 donor slice:** kinematika, PI/feed-forward és speed-map csak offline
   karakterizáció után; továbbra is fake writer.
4. **Input és becslés shadow slice:** L0–L4 adapterek, estimator és replay valódi
   capture-ökön, fizikai output nélkül.
5. **Mission/navigation slice:** L5–L9 headless commandtól constrained motionig,
   célzott scenario replayekkel.
6. **Teljes fake-only composition:** minden layer egy processzben, hosszú
   determinisztikus replay és fault injection.
7. **Read-only shadow:** legacy runtime mellett külön processzben, közös mutable
   state és writer nélkül.
8. **Külön emberi cutover:** friss preflight, IDLE/PWM-null, egyetlen writer
   gépi ellenőrzése és explicit promotion után.
