...
def test_resident_runtime_rearms_then_runs_active_and_signal_shutdown_tick():
    ...
    assert [item.inputs.context.tick_id for item in records] == [0, 1, 2, 3]
    assert [item.result for item in records] == observed
    assert records[-1].inputs.lifecycle is LifecycleState.SHUTDOWN
