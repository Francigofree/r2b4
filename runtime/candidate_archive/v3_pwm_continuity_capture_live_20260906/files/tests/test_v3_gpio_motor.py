...
def test_same_direction_allow_update_changes_duty_without_pwm_cancellation():
    gpio = FakePwmGpio()
    sink = _sink(gpio)
    sink.write(
        _frame(
    ...
    assert gpio.calls == [
        ("pwm", 7, 12, 8_000, 30.0),
        ("pwm", 7, 18, 8_000, 65.0),
        ("pwm", 7, 19, 8_000, 100.0),
    ]
    assert not sink.closed
    assert not sink.failed


def test_allow_direction_change_requires_verified_break_before_make_low():
    gpio = FakePwmGpio()
    sink = _sink(gpio)
    sink.write(
        _frame(
...
def test_same_direction_duty_failure_emergency_hard_lows_without_retry():
    gpio = FakePwmGpio()
    sink = _sink(gpio)
    sink.write(
        _frame(
    ...
    assert gpio.calls[0] == ("pwm", 7, 12, 8_000, 30.0)
    assert gpio.calls.count(("pwm", 7, 12, 8_000, 30.0)) == 1
    assert ("pwm", 7, 12, 0, 0.0) in gpio.calls
    assert ("pwm", 7, 18, 0, 0.0) in gpio.calls
    assert ("pwm", 7, 19, 0, 0.0) in gpio.calls
