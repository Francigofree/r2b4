...
class FakeGpio:
    ...
    def emit(self, pin, level, *, chip=2, delivered_pin=None, tick=None):
        if tick is None:
            tick = self.next_tick
            self.next_tick += 1
...
def test_edge_history_preserves_physical_timestamps_and_is_strictly_bounded():
    gpio = FakeGpio({18: 1, 23: 0})
    owner = NativeGpioSignedCounterPair(
        gpio,
        replace(_config(), edge_history_capacity=3),
    )

    for timestamp_ns in (10, 20, 30, 40, 50):
        gpio.emit(17, 1, tick=timestamp_ns)

    snapshot = owner.left_counter.snapshot()
    assert snapshot.pulse_count == 5
    assert tuple(edge.timestamp_ns for edge in snapshot.edge_history) == (30, 40, 50)
    assert tuple(edge.pulse_count for edge in snapshot.edge_history) == (3, 4, 5)

    owner.close()


def test_nonincreasing_physical_edge_timestamp_is_diagnostic_not_motion():
    gpio = FakeGpio({18: 1, 23: 0})
    owner = NativeGpioSignedCounterPair(gpio, _config())

    gpio.emit(17, 1, tick=50)
    gpio.emit(17, 1, tick=40)

    snapshot = owner.left_counter.snapshot()
    assert snapshot.pulse_count == 1
    assert snapshot.invalid_alerts == 1
    assert tuple(edge.timestamp_ns for edge in snapshot.edge_history) == (50,)
