...
def _frame(
    ...
    *,
    ...
    if longitudinal_acceleration_mps2 is not None:
        observations.append(
            Observation(
                "imu_acceleration",
                "BNO055_IMU",
...
@pytest.mark.parametrize("initial_yaw", (0.0, math.pi / 2.0))
def test_native_acceleration_predict_matches_legacy_core(initial_yaw):
    native = NativeStateEstimator(_config())
    native(
        _frame(
            0,
    ...
    legacy = ExtendedKalmanFilter(wheel_base=0.3557, config={})
    legacy.reset(theta=initial_yaw, v=0.2)
    legacy.predict(0.4, 0.0, 0.02)
    legacy_state = legacy.get_state()

    assert native_estimate.x_m == pytest.approx(legacy_state["x"])
    assert native_estimate.y_m == pytest.approx(legacy_state["y"])
    assert native_estimate.v_mps == pytest.approx(legacy_state["v"])
...
def test_native_config_is_immutable_and_input_validation_fails_closed():
    ...
    with pytest.raises(ValueError, match="max_abs_longitudinal_acceleration"):
        _config(max_abs_longitudinal_acceleration_mps2=0.0)
    ...
    with pytest.raises(ValueError, match="imu_acceleration"):
        estimator(
            _frame(
                0,
                left_mps=0.0,
                right_mps=0.0,
                yaw_rad=0.0,
                longitudinal_acceleration_mps2=5.1,
            )
        )
