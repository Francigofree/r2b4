...
def _sources(
    ...
    *,
    ...
    imu_acceleration_mps2: float | None = 0.4,
    ...
    imu = _Backend(
        ...
        lambda read_context: ImuHeadingReading(
            ...
            acceleration=(
                None
                if imu_acceleration_mps2 is None
                else ImuAccelerationReading(imu_acceleration_mps2)
            ),
        ),
        order,
    )
...
def test_live_input_composition_polls_three_sources_once_and_commits_one_stop():
    ...
    assert "imu_acceleration" in {
        item.kind for item in _layer(result, "L2").accepted
    }
