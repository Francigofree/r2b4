...
11. **Live input és ACTIVE-ready runtime — folyamatban:** a phase-10 kapuk
    ...
    yaw-wrap és measurement-reuse regresszió fedi.
    A nyolcadik szelet a következő hiányzó, közvetlen szenzoreredetű EKF-inputot
    adja: ugyanaz az egy immutable IMU read opcionális `ImuAccelerationReading`
    értékben a robot body `+X`, vagyis előre tengelyű gyorsulást is hordozhatja
    m/s²-ben. A native edge azonos sequence- és capture-time értékkel külön
    `imu_acceleration` sample-t zár; timing-invalid vagy stale readből nem készít
    gyorsulásmintát, a konfigurált 5 m/s² abszolút fizikai határt sértő érték
    pedig fail-closed source hiba. A `NativeStateEstimator` az admitted mintát
    `v_new = v + a*dt` és átlagsebességes pose-predict útján használja, ugyanazzal
    a Jacobian/kovariancia-propagációval, amelyet a tiszta legacy mag közvetlen
    karakterizációja fed. Hiányzó mintánál a korábbi constant-velocity út marad;
    első tick vagy gap nem integrál gyorsulást, fizikai tartományon kívüli
    előrejelzett sebesség L3 hibára zár. Az aktív `q_scale_accel=0` miatt a szelet
    nem vezet be új adaptív Q-authorityt. Command-context továbbra sem kerül
    vissza L3-ba; ehhez később külön, réteghatárt megőrző döntés kell. Nincs
    production `middleware`, NumPy, shared state, API/runtime dependency,
    falióra, thread, processz, hardverimport, `ACTIVE` vagy nem nulla writer út.
12. **Első kontrollált V3 fizikai motion és navigation — tervezett:** külön
    emberi kapuval, bounded profillal, friss preflighttal és végső PWM-nullal.
