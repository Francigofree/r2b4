...
def _print_status(controller: OperatorController, *, diagnostic: bool) -> None:
    ...
    if isinstance(status, dict):
        ...
        if status.get("state") == "RUNNING":
            ...
            if diagnostic:
                print(f"fault:   {status.get('fault_layer') or '-'}")
                for health in status.get("source_health") or []:
                    reason = f" ({health['reason']})" if health.get("reason") else ""
                    print(f"{health.get('device_id', '?')}: {health.get('state', '?')}{reason}")
