...
def _payload(
    revision=1,
    *,
    command_id="resident-test-command",
    issued=1_000,
    expires=101_000,
    mode="TELEOP",
):
    ...
    if mode == "TELEOP":
        ...
        payload.update({"max_v_mps": 0.3, "max_omega_rad_s": 0.6})
    return payload
...
def test_stop_revision_and_strict_motion_limits_close_to_command_request(tmp_path):
    ...
    active = gateway.snapshot(TickContext(1, 2_000))
    assert active.mode is CommandMode.TELEOP
    assert active.command_id == "resident-test-command"
...
def test_resident_gateway_admits_bounded_explore_without_a_second_control_path(tmp_path):
    path = tmp_path / "command.json"
    _write(path, _payload(mode="EXPLORE"))

    command = _gateway(path).snapshot(TickContext(0, 1_000))

    assert command.mode is CommandMode.EXPLORE
    assert command.command_id == "resident-test-command"
    assert _goal(command) == {"max_v_mps": 0.3, "max_omega_rad_s": 0.6}
...
def test_client_heartbeats_advance_revision_without_changing_logical_command_id(tmp_path):
    path = tmp_path / "command.json"
    times = iter((10_000, 110_000))
    ...
    gateway = _gateway(
        path,
        observed_ns=iter((20_000, 120_000)).__next__,
        maximum_ttl_ns=200_000,
        maximum_future_skew_ns=0,
    )

    first_revision = client.publish_explore(
        "room-cruise-session",
        max_v_mps=0.3,
        max_omega_rad_s=0.6,
        ttl_ns=150_000,
    )
    first = gateway.snapshot(TickContext(1, 20_000))
    second_revision = client.publish_explore(
        "room-cruise-session",
        max_v_mps=0.3,
        max_omega_rad_s=0.6,
        ttl_ns=150_000,
    ...
    assert first.command_id == second.command_id == "room-cruise-session"
    assert first_mission.mission_id == second_mission.mission_id
    assert first.mode is second.mode is CommandMode.EXPLORE
