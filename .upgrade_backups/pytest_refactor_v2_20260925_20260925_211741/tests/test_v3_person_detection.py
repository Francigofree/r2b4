...
class _Backend:
    ...
    def detect(self, frame):
        ...
        return (
            PersonDetection(0.8, PersonBox(0.1, 0.2, 0.9, 0.7)),
            PersonDetection(0.6, PersonBox(0.2, 0.1, 0.8, 0.5)),
        )
...
class _FakeInterpreter:
    ...
    def get_tensor(self, index):
        values = {
            2: np.array([[[0.1, 0.2, 0.9, 0.8], [0.0, 0.0, 0.5, 0.5], [0.2, 0.3, 0.6, 0.7]]], dtype=np.float32),
            3: np.array([[0.0, 3.0, 0.0]], dtype=np.float32),
            4: np.array([[0.91, 0.99, 0.20]], dtype=np.float32),
            5: np.array([3.0], dtype=np.float32),
        }
        return values[index]
...
def test_litert_backend_filters_person_class_and_threshold(tmp_path: Path):
    ...
    detector = LiteRtSsdPersonDetector(
        ...
        interpreter_factory=_FakeInterpreter,
    )
    frame = _Frame(1, 10)
...
def test_picamera2_rgb888_memory_is_reordered_to_model_rgb(tmp_path: Path):
    ...
    detector = LiteRtSsdPersonDetector(
        LiteRtPersonDetectorConfig(model_path=str(model)),
        interpreter_factory=CapturingInterpreter,
    )
    # Picamera2 RGB888 is B,G,R in memory. First visible pixel is therefore
    # B=10,G=20,R=30 and must reach the model as RGB [30,20,10].
...
def test_stop_timeout_does_not_allow_second_worker():
    camera = _Camera()
    ...
    detector = NativePersonDetector(camera, BlockingBackend(), stop_join_timeout_s=0.01)
    assert detector.start()
    camera.publish(_Frame(1, 1_000))
    assert entered.wait(0.5)
    detector.stop()
    assert detector.get_detection_status().last_error == "PERSON_DETECTOR_STOP_TIMEOUT"
    assert detector.start() is False
    release.set()
    deadline = time.monotonic() + 1.0
    while detector._thread is not None and time.monotonic() < deadline:
        time.sleep(0.005)
