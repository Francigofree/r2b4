...
def test_follow_search_target_is_completion_driven_not_timer_flipped():
    ...
    # SEARCH must expose CENTER for at least one evaluation. This preserves the
    # existing P0 recovery contract and prevents an already-centered robot from
    # skipping directly to +SWEEP on the entry tick.
    c3 = TickContext(3, c2.monotonic_ns + config.follow_person_lost_hold_ns + 1)
    center = nav.evaluate(_mission(c3), _estimate(c3, 0.0), _world(c3, _person("person-2")))
    assert center.route[0].yaw_rad == pytest.approx(0.0)

    # Time alone must not advance the search if CENTER has not been reached.
    c4 = TickContext(4, c3.monotonic_ns + config.follow_person_search_step_ns + 100_000_000)
    still_center = nav.evaluate(_mission(c4), _estimate(c4, 0.1), _world(c4, _person("person-2")))
    assert still_center.route[0].yaw_rad == pytest.approx(0.0)

    # Reaching CENTER advances to +SWEEP.
    c5 = TickContext(5, c4.monotonic_ns + 100_000_000)
    right = nav.evaluate(_mission(c5), _estimate(c5, 0.0), _world(c5, _person("person-2")))
    assert right.route[0].yaw_rad == pytest.approx(config.follow_person_search_sweep_rad)

    # Again, elapsed time cannot flip the target while the yaw is incomplete.
    c6 = TickContext(6, c5.monotonic_ns + config.follow_person_search_step_ns + 100_000_000)
    still_right = nav.evaluate(_mission(c6), _estimate(c6, 0.1), _world(c6, _person("person-2")))
    assert still_right.route[0].yaw_rad == pytest.approx(config.follow_person_search_sweep_rad)

    # Reaching +SWEEP advances to -SWEEP.
    c7 = TickContext(7, c6.monotonic_ns + 100_000_000)
    left = nav.evaluate(_mission(c7), _estimate(c7, config.follow_person_search_sweep_rad), _world(c7, _person("person-2")))
