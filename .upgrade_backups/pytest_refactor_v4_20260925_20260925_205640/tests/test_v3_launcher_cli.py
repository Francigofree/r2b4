...
def test_launcher_delegates_robot_commands_to_canonical_interface(monkeypatch):
    calls = []
    monkeypatch.setattr(launcher_cli, "project_root", lambda: (Path(__import__("os").environ["R2B4_ROOT"]).resolve() if __import__("os").environ.get("R2B4_ROOT") else next((p for p in Path(__file__).resolve().parents if (p / "conf" / "hardver.json").is_file() and (p / "v3").is_dir()), Path.cwd())))
