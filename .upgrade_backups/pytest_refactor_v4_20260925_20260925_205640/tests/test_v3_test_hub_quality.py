...
def test_testhub_pytest_scope_executes_quality_tests(monkeypatch, tmp_path):
    ...
    result = portable.run_pytest((Path(__import__("os").environ["R2B4_ROOT"]).resolve() if __import__("os").environ.get("R2B4_ROOT") else next((p for p in Path(__file__).resolve().parents if (p / "conf" / "hardver.json").is_file() and (p / "v3").is_dir()), Path.cwd())), scope="testhub")
