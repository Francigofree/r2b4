...
Developer tools:
  ...
  r NAME [args...]              direct-run a matching repo tool if NAME.py exists
...
case "$CMD" in
    ...
    host)
        ...
        ;;
    *)
        # Direct convenience path for repo tools: "r v3_sensor_measurement ..."
        if TOOL_PATH="$(find_tool "$CMD" 2>/dev/null)"; then
            cd "$ROOT" || exit 2
            exec python3 "$TOOL_PATH" "$@"
        fi
        # Everything else remains on the existing canonical robot interface.
        need_root || exit $?
        cd "$ROOT" || exit 2
