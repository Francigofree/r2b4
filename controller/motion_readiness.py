...
class MotionSemanticsEngine:
    ...
    def enforce(self, ctrl, ekf_state: Optional[Dict[str, Any]] = None, now: Optional[float] = None) -> Dict[str, Any]:
        ...
        heading_error_deg = 0.0
        heading_hold_applied = False
        heading_hold_deferred_to_executor = False
        ...
        if deterministic_straight_twist and abs(float(omega_target)) > 1e-9:
            # Keep straight-chain SSOT deterministic: clear carried omega so executor
            # can own heading correction without reclassifying command as curved.
            ctrl.omega_target = 0.0
            omega_target = 0.0
            semantic = _semantic_from_targets(v_target, omega_target)
            actions.append("STRAIGHT_GATE_OMEGA_ZEROED")
        executor_straight_hold_candidate = bool(
            deterministic_straight_gate_active
            or (
                execution_mode_now == "TWIST_EXEC"
                and (resolved_command_type == "set_twist" or active_command_type == "set_twist")
                and abs(requested_v) > self.v_eps
                and abs(requested_omega) <= self.w_eps
                and abs(v_target) > self.v_eps
                and abs(omega_target) <= self.w_eps
            )
        )
        explicit_reverse_straight = bool(
        ...
        if self.forward_heading_hold_enable and explicit_reverse_straight:
            heading_hold_deferred_to_executor = True
            heading_hold_mode = "EXECUTOR_DEFERRED_REVERSE"
            self._forward_ref_heading_deg = None
            actions.append("REVERSE_HEADING_HOLD_DEFERRED")
        elif (
            self.forward_heading_hold_enable
            ...
            if executor_straight_hold_candidate:
                heading_hold_deferred_to_executor = True
                heading_hold_mode = "EXECUTOR_DEFERRED"
                self._forward_ref_heading_deg = None
                actions.append("FORWARD_HEADING_HOLD_DEFERRED")
            else:
                heading_deg = _safe_float(ekf_state.get("theta_deg"), 0.0)
                ...
                if abs(omega_target) <= self.w_eps:
                    ...
                    omega_target = float(omega_corr)
                    heading_hold_applied = True
                    heading_hold_mode = "SEMANTICS_APPLIED"
                else:
                    heading_hold_mode = "SEMANTICS_BYPASSED_EXPLICIT_TURN"
        else:
            self._forward_ref_heading_deg = None
        ...
        correction_active = bool(
            ...
            or heading_hold_deferred_to_executor
        ...
        status = {
            ...
            "heading_error_deg": round(float(heading_error_deg), 4),
            "heading_hold_applied": bool(heading_hold_applied),
            "heading_hold_deferred_to_executor": bool(heading_hold_deferred_to_executor),
            "heading_hold_mode": str(heading_hold_mode),
            "executor_straight_hold_candidate": bool(executor_straight_hold_candidate),
            "deterministic_straight_gate_active": bool(deterministic_straight_gate_active),
            "executor_straight_hold_command_type": str(
                resolved_command_type or active_command_type or ""
            ),
...
class BehaviorMotionInterface:
    ...
    def rotate_to_heading(
        ...
        *,
        ...
        if getattr(self.ctrl, "heading_controller", None) is not None:
            try:
                self.ctrl.heading_controller.start(
                    target_heading_deg=float(target),
                    current_heading_deg=float(curr_heading),
                    pose_x=float(curr.get("x", 0.0)),
                    pose_y=float(curr.get("y", 0.0)),
                    source=str(source or "STATE"),
                    settle_tolerance_deg=tolerance_deg,
        ...
        self.ctrl.behavior_motion_status = {
            ...
            "speed_level": (
                ...
                else int(getattr(getattr(self.ctrl, "heading_controller", None), "default_speed_level", 1))
            ),
        }
        self.ctrl.motion_public_target = {
            "target_distance_m": None,
    ...
    def heading_error(self) -> float:
        if getattr(self.ctrl, "heading_controller", None):
            hs = self.ctrl.heading_controller.status()
