...
def inspect_mcap(capture_path: str | Path, *, deep: bool = False) -> dict[str, object]:
    ...
    return {
        ...
        "status": "PASS" if structure.valid else "FAIL",
...
def diagnose_run(
    ...
    *,
    ...
    inspect = inspect_mcap(capture_path, deep=False)
    ...
    diagnosis = _build_diagnosis(inspect, triage, replay, replay_error)
        ...
        if isinstance(tick, int):
            try:
                replay_result = replay_mcap(
                    capture_path,
                    window=ReplayWindow(
                        requested_start_tick_id=max(0, tick - 2),
                        requested_end_tick_id=tick + 2,
                    ),
                    project_root=project_root,
                )
            except Exception as exc:  # agent brief must report inability, not crash silently
                replay_result = {"status": "ERROR", "error": str(exc)}
    diagnosis = _build_diagnosis(inspect, triage, replay_result, None)
...
def verify_evidence(index_path: str | Path) -> dict[str, object]:
    ...
    return {
        "status": "PASS" if checksum_ok and all_ok else "FAIL",
...
def _build_diagnosis(
    ...
    if replay_status is None:
        ...
        replay_gate_ok = replay_status == "MATCH"
