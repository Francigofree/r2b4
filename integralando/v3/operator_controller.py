...
from v3.mcap_reader import EVENT_TOPIC, McapReadError, McapReader
...
class OperatorController:
    ...
    def wheel_targets_to_twist(
        ...
        max_omega = min(1.20, max(abs(omega), 1e-6) + 0.60)
    ...
    def capture_status(self) -> dict[str, object]:
        ...
        if path.is_file():
            try:
                ...
                complete = final.get("complete") is True
    ...
    def _run_proba_phase(self, index: int, label: str, v: float, omega: float, angle: float) -> None:
        ...
        initial = self._healthy_status()
        if not self._stopped(initial):
            raise OperatorError("phase requires ready IDLE")
    ...
    def _proba_idle(self, pause: float) -> None:
        ...
        while time.monotonic() < deadline:
            if not self._stopped(self._healthy_status()):
                raise OperatorError("runtime left IDLE during pause")
    ...
    def _capture_ready(self, path: Path) -> bool:
        ...
        try:
            ...
            return False
        return final.get("complete") is True
    ...
    def wait_idle(self, timeout: float = 3.0) -> None:
        ...
        try:
            ...
            issued_ns = int(command["issued_monotonic_ns"])
        except (OSError, KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            raise OperatorError(f"cannot verify STOP command before IDLE wait: {exc}") from exc
        if command.get("mode") != "STOP":
            raise OperatorError("another command replaced STOP before IDLE confirmation")
        ...
        while time.monotonic() < deadline:
            ...
            if (
                ...
                and int(status.get("monotonic_ns", 0)) > issued_ns
                and status.get("ready_for_active") is True
                and status.get("safety_decision") == "STOP"
    ...
    def _healthy_status(self) -> dict[str, object]:
        status = self._read_status_optional()
        if status is None:
        ...
        if (
            ...
            or any(isinstance(item, Mapping) and item.get("state") != "OK" for item in source_health)
        ):
            raise OperatorError(f"runtime/health/timing failure: {status}")
        return status
    ...
    def _publish_stop(self) -> None:
        client = ResidentCommandClient(ResidentCommandMailboxConfig(path=self.command_file))
        client.publish_stop(f"operator-stop-{time.time_ns()}-{os.getpid()}", ttl_ns=200_000_000)
    ...
    def _runtime_pid(self) -> int | None:
        ...
        if pid is not None and self._pid_matches(pid, ("v3_process_runtime.py",)):
            return pid
        self._unlink(self.runtime_pid_file)
        pid = self._find_pid(("v3_process_runtime.py",))
        if pid is not None:
            self._write_private_text(self.runtime_pid_file, str(pid))
        return pid
