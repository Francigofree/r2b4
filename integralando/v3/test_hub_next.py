...
from .test_hub_v2 import diagnose_run, verify_evidence
...
def _diagnose_once(capture: Path, output_dir: Path, replay_mode: str) -> dict[str, object]:
    index = output_dir / "evidence_index.json"
    if index.is_file():
        verified = verify_evidence(index)
        if verified.get("status") == "PASS":
            return {
                "status": "REUSED",
                "output_dir": str(output_dir.resolve()),
                "evidence_index": str(index.resolve()),
            }
        # A corrupt/stale derived directory must never be silently reused.
        raise RuntimeError(
            f"existing evidence failed verification: {index}; remove it or choose --output-dir"
        )
