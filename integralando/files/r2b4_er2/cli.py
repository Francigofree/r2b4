...
def _parser() -> argparse.ArgumentParser:
    ...
    status = sub.add_parser("status", help="show ER2 models, API-key presence and R2B4 interface state")
...
def main(argv: Sequence[str] | None = None, *, project_root: str | Path | None = None) -> int:
    ...
    if args.command == "status":
        ...
        data = {
            ...
            "media_socket": str(VisionMediaClient(timeout_s=cfg.media_timeout_s).socket_path),
            "operator": interface.read("operator.status"),
        }
        if args.json:
            ...
            print(f"Media socket:  {data['media_socket']}")
            print("R2B4 operator:  " + json.dumps(data["operator"], ensure_ascii=False, default=str))
        return 0

    tools = Er2RobotTools.from_interface(interface, cfg)
    ...
    try:
        if args.command == "preview":
            ...
            result = Er2PreviewClient(cfg).run(
                args.prompt,
        ...
        if args.command == "stream":
            ...
            result = Er2StreamingClient(tools, media, cfg).run(args.task, duration_s=args.seconds)
            print()
...
__all__ = ["main", "PREVIEW_MODEL", "STREAMING_MODEL"]
