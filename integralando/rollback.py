...
def main() -> int:
    ...
    if not manifest_path.exists():
        raise SystemExit("ERROR: no installer backup manifest found.")
    ...
    for rel, info in manifest["files"].items():
        ...
        if target.exists() and not args.force:
            ...
            if current != info["after_sha256"]:
                raise SystemExit(
                    f"ERROR: {rel} changed after installation; refusing rollback. "
                    "Inspect it or use --force."
                )

    for rel, info in manifest["files"].items():
        src = backup / info["backup"]
        dst = repo / rel
        shutil.copy2(src, dst)
        if _sha256(dst) != info["before_sha256"]:
