...
R2B4_HRI_P0_V1: successful execution returns a compact action receipt carrying
command/mission correlation without creating any new robot authority.
"""
...
class VoiceActionExecutor:
    ...
    def execute_proposal(self, proposal: object) -> VoiceActionExecution:
        ...
        # L5's current canonical mission identity is mission-{command_id}. If the
        # adapter did not return the mission id yet, expose the deterministic L5
        # identity and let Test Hub verify it against captured L5 evidence.
        if mission_id is None and command_id is not None and action.name != "v3.command.stop":
