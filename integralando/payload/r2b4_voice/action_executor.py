"""Fresh-state executor for catalog-described R2B4 robot actions.

The LLM remains a proposal source only. This executor performs a fresh
RobotInterface/context check immediately before delegating to the canonical V3
command interface. It never receives a runtime, motor, GPIO or L12 handle.
"""
...
class VoiceActionExecutor:
    """Validate from the canonical catalog and execute through RobotInterface."""
    ...
    def execute_proposal(self, proposal: object) -> VoiceActionExecution:
        ...
        if descriptor is None or descriptor.voice_exposed is not True:
            return VoiceActionExecution("REJECTED:ACTION_NOT_CATALOGED", action.name, False)

        parameters: dict[str, object] = action.as_dict()
        if descriptor.session_watchdog:
            parameters["session_owner_pid"] = self._owner_pid
            parameters["session_watchdog_s"] = self._watchdog_s
        raw_result = self._interface.execute(action.name, **parameters)
    ...
    @staticmethod
    def _correlation_ids(result: object) -> tuple[str | None, str | None]:
        if isinstance(result, Mapping):
            command = result.get("command_id")
            mission = result.get("mission_id")
        else:
            command = getattr(result, "command_id", None)
            mission = getattr(result, "mission_id", None)
        command_id = command if isinstance(command, str) and command else None
        mission_id = mission if isinstance(mission, str) and mission else None
        return command_id, mission_id
