...
EXPECTED_GIT_BLOBS = {'v3/layers/l6_navigation.py': 'fc6465cd83483bc89d656c9e950a68fe3989c07a', 'v3/composition/native_control.py': 'cbcf40e0a60481843033fa352fef86d761e16363', 'conf/vezerles.json': '699067c80814e44f190bc2de4cd6873672094df8', 'STRUKTURALIS_RETEGEK_V3.md': 'f7aca7b7d5e9077a927b19203e79aeb59a372888', 'tests/test_v3_async_l6_planner.py': '85b93f1a05a37390d2dd6231e0d62a0c037f295d'}
...
def build_updated_sources(repo: Path) -> tuple[dict[str, str], dict[str, str]]:
    current: dict[str, str] = {}
    updated: dict[str, str] = {}

    for rel, expected_blob in EXPECTED_GIT_BLOBS.items():
        actual_blob = run(repo, "git", "hash-object", rel)
        if actual_blob != expected_blob:
            raise SystemExit(
                f"ERROR: {rel} is not the inspected source version. "
                f"expected git blob {expected_blob}, actual {actual_blob}. "
                "No files changed."
            )
    ...
    for rel, replacements in PATCHES.items():
        ...
        for index, (old, new) in enumerate(replacements, start=1):
            ...
            if count != 1:
                ...
                raise SystemExit(
                    f"ERROR: anchor #{index} for {rel} matched {count} times "
        ...
        if rel == "tests/test_v3_async_l6_planner.py":
            if TEST_MARKER in changed:
                raise SystemExit(
                    "ERROR: timebase regression tests already exist while the "
                    "source is still at the pre-upgrade blob. Partial state detected."
                )
            changed = changed.rstrip() + TEST_APPEND + "\n"

        # In-memory syntax/format checks before any write.
        if rel.endswith(".py"):
            compile(changed, rel, "exec")
        if rel.endswith(".json"):
            parsed = json.loads(changed)
            async_l6 = parsed["v3_navigation"]["async_l6"]
            if async_l6.get("release_delay_ns") != 100_000_000:
                raise SystemExit("ERROR: generated config lacks 100 ms release_delay_ns")
            if async_l6.get("max_plan_age_ns") != 350_000_000:
                raise SystemExit("ERROR: generated config changed max_plan_age_ns")

        updated[rel] = changed

    l6 = updated["v3/layers/l6_navigation.py"]
    required = (
        "pending_release_not_before_ns",
        "release_delay_ns",
        "context.monotonic_ns < release_not_before_ns",
        "ASYNC_L6_DEADLINE_MISSED",
        "ASYNC_L6_PLAN_STALE",
    )
    missing = [item for item in required if item not in l6]
    if missing:
        raise SystemExit("ERROR: generated L6 is incomplete: " + ", ".join(missing))
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="verify exact source and build the patch in memory without writing files",
    )
    ...
    current, updated = build_updated_sources(repo)

    if args.check:
        print("PASS: exact inspected source matches")
        print(f"HEAD: {head}")
        print("PASS: all anchors unique")
        print("PASS: generated Python/JSON validates in memory")
        print("PASS: no files changed (--check)")
        return 0
    ...
    manifest = {
        "schema": "R2B4_ASYNC_L6_TIMEBASE_FIX_BACKUP_V2",
    ...
    print("INSTALLED: async-L6 monotonic handoff P0 fix V2")
    print(f"repo: {repo}")
    print("changed:")
    for rel in targets:
        print(f"  {rel}")
