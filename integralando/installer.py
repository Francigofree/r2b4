...
UPGRADE_ID = "r2b4_tick_phase_diag_p0_20260920"
DEFAULT_ROOT = Path("/home/alba/project_r2b4")
EXPECTED_GIT_BLOBS = {
    "v3/engine.py": "f9b0d3d3f166f4c38e46ec1ac64e7f3a39e1e3e6",
    "v3/composition/native_control.py": "7e00fca412aea4da3566f27fbc167d146c08e12f",
    "v3/composition/resident_live_control.py": "6e3342aa14ec7b0149faaa59e48c41bfc9b5000a",
    "v3/composition/resident_physical_control.py": "16fecd714f034cf9a7b36371f61cef9529ca2709",
    "v3/runtime_performance.py": "9de7273d444346bee776ef9d15deeed2de7b349f",
    "v3_runtime.py": "c7b3f663049c051cd26521ec6b987c373418ed5c",
    "tests/test_v3_tick_engine.py": "12c4bcef97ecb35e53f547efb948e5dec93e52b5",
}
NEW_FILES = (
...
def replace_once(text: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        fail(f"patch precondition failed for {label}: expected 1 match, got {count}")
...
def patch_engine(text: str) -> str:
    ...
    text = replace_once(text, old_finalize, new_finalize, "engine L12 timing")
    ...
    return replace_once(text, old_eval, new_eval, "engine L1-L11 timing")
...
def patch_runtime_performance(text: str) -> str:
    ...
    text = replace_once(text, marker, replacement, "runtime performance phase order")
    ...
    text = replace_once(text, old_evidence, new_evidence, "runtime phase evidence")
    text = replace_once(
        text,
        '''        "_work_over_period",\n    )\n''',
        '''        "_work_over_period",\n        "_control_phases",\n    )\n''',
...
def patch_tick_engine_test(text: str) -> str:
    ...
    if "test_passive_timing_observer_reports_layer_order_without_changing_result" in text:
        fail("tick timing tests already present")
...
def main() -> int:
    ...
    if not (root / "pytest.ini").is_file() or not (root / "v3").is_dir():
        fail(f"invalid R2B4 root: {root}")
    if not (root / ".git").exists():
        fail(f"R2B4 root is not a git working tree: {root}")
    ...
    try:
        for relative, patcher in PATCHERS.items():
            ...
            if updated == original:
                fail(f"patch produced no change: {relative}")
        ...
        run(sys.executable, "-m", "py_compile", *PATCHERS.keys(), *NEW_FILES, cwd=root)
        run(
