...
from .planner_capture_evidence import build_planner_capture_evidence
...
def encode_capture_record(record: CaptureRecord) -> dict[str, object]:
    ...
    if isinstance(record, ExecutionRecord):
        ...
        planner_evidence = build_planner_capture_evidence(record.result)
        if planner_evidence is not None:
            row["planner_evidence"] = encode_value(planner_evidence)
        if record.evidence:
